    ...
    <div class="main">
        ...
        <div class="content">
            <div class="card">
                ...
                <form method="get" action="<%= request.getContextPath() %>/orders/manage.jsp" class="filter-form">
                    ...
                    <a class="btn secondary" href="<%= request.getContextPath() %>/orders/manage.jsp">Xóa lọc</a>
                </form>
                ...
                <% } else { %>
                    <table>
                        ...
                        <% for (AdminOrder order : orders) { %>
                            <tr>
                                ...
                                <td>
                                    <form method="post" action="<%= request.getContextPath() %>/orders/manage.jsp" class="status-form">
                                        <input type="hidden" name="orderId" value="<%= order.getId() %>">
                                        <input type="hidden" name="keyword" value="<%= esc(keyword) %>">
                                        <input type="hidden" name="statusFilter" value="<%= selectedStatusCode %>">
                    ...
                    <% if (totalPages > 1) { %>
                        <div class="pagination">
                            <% if (currentPage > 1) { %>
                                <a class="page-link" href="<%= request.getContextPath() %>/orders/manage.jsp<%= buildQuery(keyword, selectedStatusCode, currentPage - 1) %>">Trước</a>
                            <% } %>

                            <% for (int i = 1; i <= totalPages; i++) { %>
                                <a class="page-link <%= i == currentPage ? "active" : "" %>" href="<%= request.getContextPath() %>/orders/manage.jsp<%= buildQuery(keyword, selectedStatusCode, i) %>"><%= i %></a>
                            <% } %>

                            <% if (currentPage < totalPages) { %>
                                <a class="page-link" href="<%= request.getContextPath() %>/orders/manage.jsp<%= buildQuery(keyword, selectedStatusCode, currentPage + 1) %>">Sau</a>
                            <% } %>
                        </div>
                    <% } %>
                <% } %>
