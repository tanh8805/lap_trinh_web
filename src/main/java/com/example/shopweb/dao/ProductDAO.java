...
public class ProductDAO {
        ...
        try (Connection conn = DBConnection.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    // Lấy thêm map size → price từ product_variants
                    p.setVariantPrices(getVariantPrices(id));
                    return p;
                }
            }
    ...
    /**
     * Lấy Map<size, price> của 1 sản phẩm từ product_variants. Dùng để modal
     * hiển thị đúng giá theo size được chọn.
     */
    private Map<String, Double> getVariantPrices(int productId) {
        Map<String, Double> variantPrices = new LinkedHashMap<>();
        String sql = "SELECT size, price FROM product_variants WHERE product_id = ? AND stock_quantity > 0 ORDER BY price";
